...
//        env.setStateBackend(new FsStateBackend(""));
      ...
      FlinkKafkaConsumer<String> pageViewSource = MyKafkaUtil.getFlinkKafkaCounsumer(pageViewSourceTopic, groupId);
      FlinkKafkaConsumer<String> orderWideSource = MyKafkaUtil.getFlinkKafkaCounsumer(orderWideSourceTopic, groupId);
      FlinkKafkaConsumer<String> paymentWideSource = MyKafkaUtil.getFlinkKafkaCounsumer(paymentWideSourceTopic, groupId);
      FlinkKafkaConsumer<String> favorInfoSourceSource = MyKafkaUtil.getFlinkKafkaCounsumer(favorInfoSourceTopic, groupId);
      FlinkKafkaConsumer<String> cartInfoSource = MyKafkaUtil.getFlinkKafkaCounsumer(cartInfoSourceTopic, groupId);
      FlinkKafkaConsumer<String> refundInfoSource = MyKafkaUtil.getFlinkKafkaCounsumer(refundInfoSourceTopic, groupId);
      FlinkKafkaConsumer<String> commentInfoSource = MyKafkaUtil.getFlinkKafkaCounsumer(commentInfoSourceTopic, groupId);
      ...
      SingleOutputStreamOperator<ProductStats> productStatsWithClickAndDisplayDS = pageViewDStream.flatMap(new FlatMapFunction<String, ProductStats>() {
          ...
          public void flatMap(String value, Collector<ProductStats> out) throws Exception {
              ...
              String item_type1 = page.getString("item_type");
                  ...
                  for (int i = 0; i < displays.size(); i++) {
                      ...
                      if ("sku_id".equals(display.getString("item_type"))) {
                          out.collect(ProductStats.builder()
                                  ...
                                  .display_ct(1L)
                                  .ts(ts)//TODO 这个位置导致下面获取时间的时候出现空指针
                                  .build());
                      }
                  }
              }
...
//                        System.out.println("stats2" + stats2.getRefund_amount());
                  ...
                  //TODO reduce只是对每一条数据进行一个增量聚合，window是对窗口里的数据进行一个全量聚合
                  @Override
